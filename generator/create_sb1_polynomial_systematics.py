...
# Systematics.
a2_fraction_params = np.random.uniform(0., 0.5, size=N_SYSTEMS)
n_poly_roots_params = np.random.random_integers(1, 10, size=N_SYSTEMS)
...
for i in range(N_SYSTEMS):
    ...
    # Add systematics: polynomial.
    A2 = a2_fraction_params[i] * k1_params[i]
    n_roots = n_poly_roots_params[i]
    epochs_centered = sd.jd - t0_param
    poly_roots = np.random.uniform(
        np.min(epochs_centered), np.max(epochs_centered), n_roots)
    poly_roots = np.concatenate(
        [[np.min(epochs_centered) - 1], poly_roots, [np.max(epochs_centered) + 1]])
    polynomial = np.poly1d(poly_roots, r=True)
    sys_pri = polynomial(epochs_centered)
    sys_pri *= A2 / np.max(np.abs(sys_pri))
...
with open('sb1_dataset_polynomials.p', 'wb') as f:
